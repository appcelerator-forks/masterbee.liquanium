/*===========================================
 Cacheing HTTP Client with overwrite protection
 @notes:
 ============================================*/
...
XHR.prototype.get = function(url, onSuccess, onError, extraParams) {
	...
	if (!extraParams.ttl || cache == 0) {
		...
		if (extraParams.shouldAuthenticate) {
			var authstr = 'Basic ' + Ti.Utils.base64encode(extraParams.username + ':' + extraParams.password); 
...
XHR.prototype.post = function(url, data, onSuccess, onError, extraParams) {
	...
	// Debug
	Ti.API.info(url + " " + JSON.stringify(data));
...
XHR.prototype.clear = function(url) {
	...
	if (url) {
		// Hash the URL
		var hashedURL = Ti.Utils.md5HexDigest(url);
		...
		if (cache) {
			...
			var file = Ti.Filesystem.getFile(Ti.Filesystem.applicationDataDirectory, hashedURL);
			...
			//Ti.API.info("REMOVED CACHE FILE " + hashedURL);
		}
	} 
...
XHR.prototype.clean = function() {
	...
	for (var key in cacheManager) {
		...
		if(cache.timestamp <= nowInMilliseconds){
			// Delete references and file
			var file = Ti.Filesystem.getFile(Ti.Filesystem.applicationDataDirectory, key);
			// Delete the record and file
			...
			//Ti.API.info("REMOVED CACHE FILE " + cachedDocuments[i].file);
		}
...
var readCache = function(url) {
	...
	if (cache) {
		...
		if (cache.timestamp >= new Date().getTime()) {
			//Ti.API.info("CACHE FOUND");

			// Return the content of the file
			result = file.read();

		} else {
			//Ti.API.info("OLD CACHE");
		...
		//Ti.API.info("CACHE " + hashedURL + " NOT FOUND");
	}
...
var updateCacheManager = function(){
	Ti.App.Properties.setObject("cachedXHRDocuments", cacheManager);
};

var writeCache = function(data, url, ttl) {
		
	//Ti.API.info("WRITING CACHE");
